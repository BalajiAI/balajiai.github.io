...
subtitle: In this blog, we'll briefly discuss about the problem of high variance in Policy gradients and techniques for variance reduction.
layout: default
date: 2024-02-17
...
### Control Variates
Let's consider a random variable $X$ which has an expectation value of $\E[X]$ and a high variance of $Var[X]$. Since $X$ has a large variance, we require large number of samples to accurately compute its mean. So what control variate method proposes to solve this problem is, to construct a new random variable $X^*$ which has the same expectation value of $X$ ($\E[X^*] = \E[X]$), but lesser variance ($Var[X^*] < Var[X]$). So instead of computing sample mean of $X$ (requires large no.of.samples), it's just enough to compute the sample mean of $X^*$ (requires less no.of.samples). $X^*$ is constructed by the following means,
$$\begin{gather*}
 X^* = X - c (Y - \mu_Y)
\end{gather*}$$
Where $c$ is a constant and $Y$ is an another random variable which correlates with $X$. We'll prove that indeed $\E[X^*] = \E[X]$ and $Var[X^*] \leq Var[X]$ in the following.
$$\begin{align*}
 \E[X^*] &= \E[X - c (Y - \mu_Y)] \\
 &=\E[X] - c \E[(Y - \mu_Y)] \\
 &= \E[X]
...
$$\begin{align*}
 Var[X^*] &= \E[X^{*2}] - \E[X^*]^2 \\
 &= \E[(X - c (Y - \mu_Y))^2] - \E[X]^2 \\
 &= \E[X^2] + \E[c^2 (Y - \mu_Y)^2] - \E[2X.c(Y - \mu_Y)] - \E[X]^2 \\ 
 &= Var[X] + c^2 \E[ (Y - \mu_Y)^2] - 2c\E[X.(Y - \mu_Y)] \\  
...
$$\begin{gather*}
    ...
    Var[\nabla_{\theta} J(\theta)] = \E_{\tau \sim \pi_\theta}[(f(\tau) (R(\tau) - b))^2] - \E_{\tau \sim \pi_\theta}[f(\tau) (R(\tau) - b)]^2    
\end{gather*}$$
...
$$\begin{align*}
    \frac{d}{db} Var[\nabla_{\theta} J(\theta)] &= \frac{d}{db} \E_{\tau \sim \pi_\theta}[(f(\tau) (R(\tau) - b))^2] \\
&= \frac{d}{db} \E_{\tau \sim \pi_\theta}[f(\tau)^2 (R(\tau)^2 + b^2 - 2R(\tau)b)] \\
&= \frac{d}{db} \Bigl( \E_{\tau \sim \pi_\theta}[f(\tau)^2 R(\tau)^2] + b^2\E_{\tau \sim \pi_\theta}[f(\tau)^2] - 2b\E_{\tau \sim \pi_\theta}[f(\tau)^2R(\tau)] \Bigl) \\
&= 2b\E_{\tau \sim \pi_\theta}[f(\tau)^2] - 2\E_{\tau \sim \pi_\theta}[f(\tau)^2R(\tau)]
\end{align*}$$
Set the above result to zero and solve for $b$,
$$\begin{gather*}
    2b\E_{\tau \sim \pi_\theta}[f(\tau)^2] - 2\E_{\tau \sim \pi_\theta}[f(\tau)^2R(\tau)] = 0 \\
    b = \frac{\E_{\tau \sim \pi_\theta}[f(\tau)^2R(\tau)]}{\E_{\tau \sim \pi_\theta}[f(\tau)^2]} \\
    b = \frac{\E_{\tau \sim \pi_\theta}[(\nabla_{\theta} \log \pi_{\theta}(\tau))^2R(\tau)]}{\E_{\tau \sim \pi_\theta}[(\nabla_{\theta} \log \pi_{\theta}(\tau))^2]}    
\end{gather*}$$
...
In vanilla policy gradient, the probability of a trajectory $\pi_{\theta}(\tau)$ is incremented on par with the return $R(\tau)$ during the policy update. In policy gradient with baseline, the probability is incremented only if $R(\tau) - b$ is positive and decremented if it is negative. In other words, a trajectory's probability is pushed up only when that particular trajectory's return is greater than the average return ($R(\tau) > \E_{\tau \sim \pi_\theta} [R(\tau)]$).
...
\begin{enumerate}
  \item $Q(s_{t}, a_{t}) - V(s_{t})$ : Advantage function decomposition
  \item $r_{t} + \gamma r_{t+1} + \gamma^2 r_{t+1}+...+ \gamma^n r_{t+n} + \gamma^{n+1} V(s_{t+n+1}) - V(s_t)$ : TD(n) Error
  \item $\sum_{l=0}^{\infty} (\gamma \lambda)^{l} \delta_{t+l}$, where $\delta_{t} = r_t + \gamma V(s_{t+1}) - V(s_t)$ : Generalized Advantage Estimation (GAE)
\end{enumerate}
